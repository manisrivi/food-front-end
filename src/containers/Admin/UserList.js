  ...
  return (
    <div>
      ...
      <input
        ...
        />
      <p className="text-center text-danger fw-bold mx-auto">-All Users Information</p>
      </div>
        <div className="row table-responsive">
          ...
          <div className="text-center">  {isLoading && (
            ...
            <tbody className="bg-light">
              {users.filter((g) => g.fullname.toLowerCase().includes(query)).map((u, index) => {
                return (
                  <tr key={index}>
                    <td>{u._id}</td>
                    <td>{u.fullname}</td>
                    ...
                    <td className="d-flex gap-2">
                      ...
                      <button className="btn btn-outline-white border-0"
    ...
    <div className="container">
      ...
      <div className="row justify-content-center p-2">
        <div className="col mt-5 MainContent_Text">
          <h4>FullName: <span className="text-primary">{users.fullname}</span></h4>
          <h4>Email: <span className="text-primary">{users.email}</span></h4>
          <h4>Contact-Number: <span className="text-primary">{users.contactnumber}</span></h4>
          <h4>Create-Date: <span className="text-primary">{users.date}</span></h4>
        </div>
        <div className="col">
            <img src="https://i.pinimg.com/originals/0a/f3/c9/0af3c9613761d2d2394d99312aeba397.gif" alt="" />
        </div>
      </div>
    </div>
  );
